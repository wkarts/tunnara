...
const entry = kind === 'agent'
  ? path.join(root, 'runtime/node/bin/tunnara.mjs')
  : path.join(root, 'runtime/node/bin/tunnara-server.mjs');
...
function run(command, args, options = {}) {
  ...
  if (result.error) {
    throw new Error(`Falha ao iniciar ${command}: ${result.error.message}`, { cause: result.error });
  }
  if (result.status !== 0) {
    throw new Error(`${command} encerrou com código ${result.status ?? 'desconhecido'}`);
  }
}
...
if (!fs.existsSync(postject)) {
  throw new Error(`CLI do postject não encontrado em ${postject}. Execute npm ci na raiz.`);
}
